// using ncnn C++ API to load and run resnet model for image classification
...
static int detect_resnet_caffe(const cv::Mat& bgr, std::vector<float>& cls_scores)
{
    printf("%s\n", __FUNCTION__);
    ncnn::Net net;
    net.opt.use_vulkan_compute = false;

    // original pretrained model from https://github.com/KaimingHe/deep-residual-networks
    // the backup of the original pretrained resnet: https://pan.baidu.com/s/1muvGkTr8X9fdC2Ke_9NiCQ?pwd=3w6r pswd: 3w6r
    // convert it to ncnn by using `bin/caffe2ncnn.exe` from latest prebuilt ncnn zip: https://github.com/Tencent/ncnn/releases
    // or convert it from https://convertmodel.com/ , not choose ncnnoptimize, not choose fp16
    if (net.load_param("ResNet-50-model-sim.param"))
        exit(-1);
    if (net.load_model("ResNet-50-model-sim.bin"))
        exit(-1);
...
int main(int argc, char** argv)
{
    ...
    time_t t = time(0);
    char date[64] = { 0 };
    strftime(date, sizeof(date), "%Y/%m/%d %X %A",localtime(&t) );
    printf("%s\n", date);

    printf("Inference of ResNet-50 with ncnn (version: %s)\n", NCNN_VERSION_STRING);
    ...
    using detect_resnet_func = std::function<int(const cv::Mat&, std::vector<float>&)>;
    //for (detect_resnet_func detect_resnet : {detect_resnet_caffe, detect_resnet_pnnx})
    for (detect_resnet_func detect_resnet : {detect_resnet_caffe})
    {
        fprintf(stdout, "----------\n");
